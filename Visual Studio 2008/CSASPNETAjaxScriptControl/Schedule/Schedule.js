...
CSASPNETAjaxScriptControl.Schedule = function(element) {
    ...
    this._loadingImg = null;// 所有的droppanel只应用一个loadingimg
}
...
CSASPNETAjaxScriptControl.Schedule.prototype = {
    initialize: function() {
        ...
        for (var m = 0; m < dropPanel.length; m++) {
            ...
            setTimeout("$find('" + id + "').add_closed(CSASPNETAjaxScriptControl$Schedule$ClosedCallBack)", 0); // 回调静态方法
        }
        ...
        if (aa != null) {
            ...
            if (activeDropPanel != null && this._updateWebRequest == null) {
                ...
                if (obj != null) {
                    ...
                    //如果光标在mouseout和mouseover时保持在droppanel上, 
                    //mouseout不会被触发. 因此我们可以用settimeout延迟 
                    //droppanel移动以确保mouseout被触发.
...
//**** 要建立一个子脚本控制客户端事件, 我们必须使用settime设定回调, 因为子脚本控制会在此控件之后被注册.
//**** 然后我们必须建立静态方法实现子脚本控制的客户端事件的回调
//**** 这个静态方法将再次回调类的原型方,因此看上去像droppanel关闭事件直接调用schedule类原型的回调函数.
//**** 事实上, 我们必须通过静态方法调用, 除非我们想要在渲染阶段设定脚本控件的注册顺序.
function CSASPNETAjaxScriptControl$Schedule$ClosedCallBack(sender,e) {
    ...
    activeSchedule._dropPanelClosed(sender, e);// 这里指向窗体
}
