========================================================================
    控制台应用程序 ： CSEFForeignKeyAssociation项目概述
========================================================================
...
1. 使用Foreign Key Association建立实体数据模型。  
   ...
   4) 选中数据表Course和Department。
      注意：记住要选中复选框"在模型中加入外键列"，使其允许Independent Association！

2. 创建一个静态类FKAssociationClass来测试利用外键关联的插入和更新方法。
   1) 创建方法InsertNewRelatedEntities来利用Independent Association来插入一条新的Course和Department实体。
   2) 创建方法InsertByExistingEntities来利用Independent Association来插入一条新的Course并将其设置成附属于
	  一个已存在的Department。
   3) 创建方法UpdateExistingEntities来利用Independent Association关系更新一个已存在的Course实体。
   4) 创建方法Query来查询Course实体和对应的Department实体。
   5) 创建方法Test来执行利用Independent Association的插入和更新方法。
...
2. 创建一个静态类IndependentAssociationClass来测试无关联的插入和更新方法。
