...
直接打开CSASPNETAJAXWebChat.sln,展开WebChat web应用程序节点并按F5开始测试应用程序.
...
步骤 6.  创建一个新目录, "Logic". 我们需要创建些类文件.
         * ChatManager.cs: 我们需要些静态方法通过Linq控制数据库中的数据.
		 * ChatRoom.cs: 这是通过WCF服务发送聊天室数据到客户端的DataContract.
         * Message.cs: 这是通过WCF服务发送消息数据到客户端的DataContract.
		 * RoomTalker.cs: 这是通过WCF服务发送聊天室中聊天者数据到客户端的DataContract.
...
步骤 8.  创建一个新目录, "Scripts". 右击目录单击
		 ...
		 [/CODE]
		 * csaspnetajaxwebchat是本应用程序的命名空间.
		 * transition是服务名.
		 * LeaveChatRoom是方法名.
