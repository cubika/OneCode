================================================================================
       Windows 应用程序: CSCustomIEContextMenu 概述
================================================================================
/////////////////////////////////////////////////////////////////////////////
摘要:
...
注意:
1. 在 IE8 中，可以用名为加速器或者活动的新方法，将项添加到IE上下文菜单。加速器使
   得从一个网页复制信息到另一个网页变得更为容易。同时，“将项添加到IE标准上下文菜
   单”仍然是一个在本地工作的好方法. 
...
针对 x 86 或 x64 的操作系统上的 32 位 IE ，安装 CSCustomIEContextMenuSetup(x86).msi.
CSCustomIEContextMenuSetup(x86).msi 是 CSCustomIEContextMenuSetup(x86)安装项目的
输出结果.

针对 x64 的操作系统上的 64 位 IE,安装 CSCustomIEContextMenuSetup(x64).msi.
CSCustomIEContextMenuSetup(x64).msi 是 CSCustomIEContextMenuSetup(x64)安装项目的
输出结果.


B. 卸载

针对 x 86 或 x64 的操作系统上的 32 位 IE ,卸载 CSCustomIEContextMenuSetup(x86).msi.
CSCustomIEContextMenuSetup(x86).msi 是 CSCustomIEContextMenuSetup(x86)安装项目的
输出结果.

针对 x64 的操作系统上的 64 位 IE ,卸载 CSCustomIEContextMenuSetup(x64).msi.
CSCustomIEContextMenuSetup(x64).msi 是 CSCustomIEContextMenuSetup(x64)安装项目的
输出结果.
...
步骤1. 在Visual Studio 2010 中打开该项目，设置该解决方案的平台为 x86. 确保在“配
       置管理器”中选择了生成 CSCustomIEContextMenu 和 CSCustomIEContextMenuSetup(x86).
       
        注意：如果你想在 64 位IE中运行该实例，将平台设为 x64,同时选择生成 
        CSCustomIEContextMenu 和 CSCustomIEContextMenuSetup(x64).           
        
步骤2. 生成该解决方案.
 
步骤3. 在解决方案资源管理器中，右击CSCustomIEContextMenuSetup(x86)项目，选择“安装”.
       
       在安装完成后，打开32位IE，单击工具=>管理加载项，在“管理加载项”对话框，你
	   可以发现项目“CSCustomIEContextMenu.BHOIEContextMenu".

	  

演示增加项到 IE 标准上下文菜单.

步骤4. 打开32位 IE,单击工具=>管理加载项，禁用"CSCustomIEContextMenu.BHOIEContextMenu".
       您可能需要重启 IE 以使之生效.
      
步骤5.访问 http://www.microsoft.com/. 右击该网页上的一张图像，在文本菜单上，你将看到
      “在新选项卡中打开图像”项.点击该项，IE 将会打开一个新的选项卡以打开图像.   

演示重写IE标准上下文菜单.
 
步骤6. 打开32位 IE,单击工具=>管理加载项，启用"CSCustomIEContextMenu.BHOIEContextMenu".
       您可能需要重启 IE 以使之生效. 

步骤7. 访问 http://www.microsoft.com/. 右击该网页上的一张图像，在文本菜单上，你将看到
       “在新选项卡中打开图像”项.点击该项，IE 将会打开一个新的选项卡以打开图像.
...
在 Visual Studio 2010 中, 创建一个 Visual C# / Windows / Class Library 项目命名为
"CSCustomIEContextMenu". 
...
D. 用一个安装程序项目来部署自定义上下文菜单.
  ...
  此代码示例中的 CSCustomIEContextMenuSetup(x64)），并将其 TargetPlatform 属性设置为
  x64. 
  
  尽管 TargetPlatform 属性设置为 x64,与 .msi 文件打包在一起的本地安装文件，仍然是一个 32 
  位的可执行文件. Visual Studio 将 InstallUtilLib.dll 的 32 位版本.作为 InstallUtil 
  ...
  InstallUtil 的修改.在 CSCustomIEContextMenuSetup(x64) 的项目文件夹中,您可以找到该
  脚本文件.要配置该脚本以便在post-build事件中运行, 在解决方案资源管理器中，选择 
  CSCustomIEContextMenuSetup(x64) 项目，并在“属性”窗口中查找 PostBuildEvent 属性.
  指定它的值为 
