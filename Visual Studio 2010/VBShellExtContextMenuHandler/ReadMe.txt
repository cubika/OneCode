=============================================================================
      类库: VBShellExtContextMenuHandler 项目概述
=============================================================================

/////////////////////////////////////////////////////////////////////////////
摘要:

该VB.NET代码示例演示如何创建一个.NET4.0框架下的上下文菜单外壳处理程序. 
...
在.NET4.0框架中，能够与任何其他运行库的过程中有多个运行时 Microsoft现在可以提供普遍支持托管外壳扩展VBShellExtContextMenuHandler就是这样一个托管的外壳扩展代码示例.
但是，您还是不能在低于.NET4.0的任何框架下开发托管的外壳程序。


该示列的上下文程序的 class ID (CLSID):
    {1E25BCD5-F299-496A-911D-51FB901F7F40}

	
当你在资源管理器中右击一个.vb类型的文件时候，该程序将添加一个名叫"Display File Name (VB)"的子菜单项到上下文菜单中，
...
定位到VBShellExtContextMenuHandler.dll所在的文件夹输入以下命令:

    Regasm.exe VBShellExtContextMenuHandler.dll /codebase
...
定位到VBShellExtContextMenuHandler.dll所在的文件夹输入以下命令:
    ...
    Regasm.exe VBShellExtContextMenuHandler.dll /unregister
...
步骤1.	
		如果您在 Visual Studio 2010中能成功生成工程的话,您将得到一个 VBShellExtContextMenuHandler.dll的动态链接库文件
	在Visual Studio 2010 \ Visual Studio Tools menu菜单下找到'Visual Studio Command Prompt (2010)'，如果您是
	64位的操作系统请运行 'Visual Studio x64 Win64 Command Prompt (2010)',同时请确保您具有管理员的权限.
	定位到VBShellExtContextMenuHandler.dll所在的文件夹输入以下命令:
...
步骤2.
	在Window资源管理器中找一个.vb类型的文件 (例如在同各文件下有个文件FileContextMenuExt.vb), 
右击这个文件，您在上下文菜单中将看到"Display File Name (VB)"的菜单子项,点击这个菜单子项，
您将看到FileContextMenuExt.vb文件的的完整物理路径.在Window资源管理器里您只能同能选择一个文件而不是多个文件.
...
A. 创建和设置工程
	...
	在Visual Studio 2010中, 创建一个名为"VBShellExtContextMenuHandler"的Visual Basic / Windows / Class Library 工程,
...
FileContextMenuExt.vb 文件定义上下文菜单处理程序. 必须继承IShellExtInit and IContextMenu 
接口. 在文件ShellExtLib.vb中你可以看到接口可以通过COMImport属性导入
...
只有该上下文应用程序被成功注册了，当上下文菜单显示的时候才能被实例化.
  ...
  在示例代码中, 我们插入一个 "Display File Name (VB)"子菜单项并在它的下面加一个分隔符.
...
注册的 COM 类。注册调用，ShellExtReg.RegisterShellExtContextMenuHandler 方法中，ShellExtLib.vb
将该处理程序与特定文件类型相关联。如果文件的类型是以'.'开头的，它会尝试读取 HKCR\ < 文件类型 > 键的可能的默认值包含链接的文件类型的程序 ID。如果默认值不为空，使用作为文件类型的程序 ID 进行注册。


例如, 示例文件关联了 '.vb' 类型的文件. 
如果您安装了Visual Studio 2010则注册表项HKCR\.vb下就有了一个默认的文件类型'VisualStudio.vb.10.0',所以将使用'VisualStudio.vb.10.0'
来取代HKCR\.vb下的文件类型.


    HKCR
    {
        NoRemove .vb = s 'VisualStudio.vb.10.0'
        NoRemove VisualStudio.vb.10.0
        {
            NoRemove shellex
            {
                ...
                {
                    {1E25BCD5-F299-496A-911D-51FB901F7F40} = 
                        s 'VBShellExtContextMenuHandler.FileContextMenuExt'
                }
            }
        }
    }
